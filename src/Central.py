...
if __name__ == '__main__':  # 用于定义一个只有在Python脚本作为主程序运行时才执行的代码块，而不是被导入到其他脚本中。
	simulation = Central(
		model_name			=	"LeNet5",
		dataset_name		=	"FMNIST",
		batch_size			=	1,  # batchsize：批大小。在深度学习中，一般采用SGD训练，即每次训练在训练集中取batchsize个样本训练； iteration：1个iteration等于使用batchsize个样本训练一次；
		num_epochs			=	3,  # epoch：1个epoch等于使用训练集中的全部样本训练一次；
		learning_rate_type	=	"iter_decay"
	)
	log = simulation.run()

	print(log["accuracy_per_epoch"])  # 打印出每个epoch的准确性